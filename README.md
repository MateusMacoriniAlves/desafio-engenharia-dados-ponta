...
- Outro erro pode ser na configuração do arquivo dockerfile. Na documentação oficial tem um modelo, que foi utilizado nesse projeto. Porém pode ser necessário alterar o conteúdo para ```FROM apache/airflow:3.0.1 COPY requirements.txt . RUN pip install --no-cache-dir -r requirements.txt```
